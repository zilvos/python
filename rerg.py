...
# --- Animator class for one sprite sheet ---
class SpriteAnimator(Entity):
    def __init__(self, texture, frames_x, frames_y, fps=8, **kwargs):
        mesh = Mesh(
            vertices=[(-.5, -.5, 0), (.5, -.5, 0), (.5, .5, 0), (-.5, .5, 0)],
            triangles=[(0, 1, 2), (0, 2, 3)],
            mode='triangle'
        )
        super().__init__(model=mesh, texture=texture, **kwargs)

        self.frames_x = frames_x
        self.frames_y = frames_y
        self.frame_width = 1 / frames_x
        self.frame_height = 1 / frames_y

        self.total_frames = frames_x * frames_y
        self.current_frame = 0
        self.fps = fps
        self._timer = 0

        # init UVs so first frame shows
        self._update_uv()

    def _update_uv(self):
        col = self.current_frame % self.frames_x
        row = self.current_frame // self.frames_x
        u = col * self.frame_width
        v = 1 - (row + 1) * self.frame_height

        self.model.uvs = [
            (u, v),
            (u + self.frame_width, v),
            (u + self.frame_width, v + self.frame_height),
            (u, v + self.frame_height),
        ]
        self.model.generate()

    def update(self):
        self._timer += time.dt
        if self._timer >= 1 / self.fps:
            self._timer = 0
            self.current_frame = (self.current_frame + 1) % self.total_frames
            self._update_uv()


# --- Load two separate sprite sheets ---

# --- Load sprite sheets safely ---

if not player_tex:
    print("⚠️ Could not load weiner-hotdog.png")
if not enemy_tex:
    print("⚠️ Could not load hand.png")
# --- Create two animators ---
# ---- Load textures ----
tex_hand = load_texture(r"my assets\weiner-hotdog.png")
tex_hand2 = load_texture(r"my assets\hand.png")

# ---- Create hand animations on UI ----
left_hand = SpriteAnimator(tex_hand, rows=4, cols=4, fps=8,
                           scale=0.3, position=(-0.4, -0.4))
right_hand = SpriteAnimator(tex_hand2, rows=4, cols=4, fps=8,
                            scale=0.3, position=(0.4, -0.4))
...
def update():
    ...
    if 'update' in globals() and callable(globals()['update']):
        old_update = globals()['update']
        def combined_update():
            old_update()
            sprite_player.look_at(camera.world_position)
            sprite_enemy.look_at(camera.world_position)
        globals()['update'] = combined_update
    else:
        def only_update():
            sprite_player.look_at(camera.world_position)
            sprite_enemy.look_at(camera.world_position)
        globals()['update'] = only_update
